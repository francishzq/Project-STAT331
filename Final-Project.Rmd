...
author: "Francis Z. Han; Catherine Wang"
date: "03/12/2018"
fontsize: 12pt
linkcolor: blue
output: 
...
```{r, label = "leverage", echo = FALSE, fig.pos = "!htb", fig.height = 2.4, out.width = "\\textwidth", fig.cap = "Predicted Value vs Residuals and Leverages for M1 and M2"}
...
```{r, label = "rPMSE", echo = FALSE, fig.pos = "!htb", fig.height = 3.25, out.width = "\\textwidth", fig.cap = "Cross-validation model comparison results. Left: Root mean square prediction error (rMSPE). Right: Out-of-sample likelihood ratio statistic."}
...
```{r, label = "PRESS", echo = FALSE, fig.pos = "!htb", fig.height = 3.25, out.width = "\\textwidth",fig.cap = "Boxplot of squared PRESS statistics for models M1 and M2."}
# Figure 8
# plot setup
cex <- .8
pch <- 16
...
sd.errors <- summary(M2)$coefficients[,2]
tvalue <- summary(M2)$coefficients[,3]
pvalue <- summary(M2)$coefficients[,4]

n <- length(names(coef(M2)))
result <- NA
for (ii in 1:n) {
  result <- c(result, summary(M2)$coefficients[ii,])
}

result <- matrix(result[2:length(result)], nrow = n, byrow = TRUE)
colnames(result) <- c("parameter estimates", "standard errors", "t-value", "Pr(>|t|)")
rownames(result) <- c(1:18)
result
names(coef(M2))
```
...
```{r}
# create a new variable to combine smoke and number
# note that nobody is in the category 
# "Smoked up to half a pack but quit during pregnancy"
...
```{r}
# Combine mht and mwt to create a new variable BMI
# BMI is called Body Mass Index, 
# which is calculated by weight in kg / height^2 in m
# Convert mht and mwt to standard metric
lbs_to_kg <- 0.453592
birth$mwt <- lbs_to_kg * birth$mwt
inch_to_meter <- 0.0254
...
med2 <- birth$med
# "levels" of variable med
mednames <- c("elementary school", "middle school", "high school", 
              "high school + trade", "high school + some college", 
              "college grad", "trade school", "high school unclear")
med2 <- mednames[birth$med+1]
med2 <- factor(med2, levels = mednames) # order levels based on mednames
levels(med2)
birth$med <- med2 # replace in dataset
...
fed2 <- birth$fed
# "levels" of variable fed
fednames <- c("elementary school", "middle school", "high school", 
              "high school + trade", "high school + some college", 
              "college grad", "trade school", "high school unclear")
fed2 <- fednames[birth$fed+1]
fed2 <- factor(fed2, levels = fednames) # order levels based on fednames
...
missing <- c(ind_gestation, ind_mage, ind_BMI, 
             ind_marital, ind_fage, ind_fed, ind_med)
missing
...
## Mfwd, Mback and Mstep {#threemodels}
```{r}
Mfwd$call
Mback$call
Mstep$call
